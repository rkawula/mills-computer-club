Given(/^that there is a blog post titled "([^"]*)", by user id "([^"]*)"$/) do |title, author|
	@post = Post.create title: title, subtitle: "Default",
						contents: "Default", published: true,
						user_id: author
end

Given(/^that there is a user with id "([^"]*)", named "([^"]*)"$/) do |id, name|
...
Given(/^that there is a blog post titled "([^"]*)", by user id "([^"]*)", with contents "([^"]*)"$/) do |title, id, content|
